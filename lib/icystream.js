...
function getStreamStation(url, callback, teardownWhenDone, waitTime) {
	...
	if(teardownWhenDone) client.on('data', dataCallback);
		...
		interval = setInterval(function() {
			if(oldData === dataCallBacked) return;
			oldData = dataCallBacked;
	...
	function dataCallback(response) {
		...
		if (buffer.length > maxBufferSize) {
			if(teardownWhenDone) return returnError();
			else buffer = '';
		}
	}
